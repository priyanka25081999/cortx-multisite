#!/usr/bin/env python3

#
# Copyright (c) 2021 Seagate Technology LLC and/or its Affiliates
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#    http://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.
#
# For any questions about this software or licensing,
# please email opensource@seagate.com or cortx-questions@seagate.com.

import aiohttp
import asyncio
import sys
import urllib
from os.path import abspath, join, dirname
from os import urandom
from s3replicationcommon.aws_v4_signer import AWSV4Signer

# Import config module from '../tests/system'
sys.path.append(abspath(join(dirname(__file__),'..','tests', 'system')))
from config import Config

async def data_generator(object_sz, part_number):
    total_chunks = int(object_sz / part_number)
    start_chunk_size = 0
    part_no = 1
    all_objects = []
    print("Total chunks {}".format(total_chunks))
    
    while start_chunk_size < object_sz:
        local_data = None
        
        # for _ in range(start_chunk_size, start_chunk_size+total_chunks):
        #     urandom(local_data)

        local_data = urandom(total_chunks)
        
        local_dict = {}
        # local_dict['part_no'] = part_no
        # local_dict['data'] = local_data
        all_objects.append({"part_no":part_no, "data":local_data})
        local_dict.clear()

        start_chunk_size+=total_chunks
        local_data = None
        part_no+=1

    yield all_objects

async def main():
    async with aiohttp.ClientSession() as session:
        config = Config()

        bucket_name = config.source_bucket_name
        object_name = config.object_name_prefix
        part_count = config.part_number

        request_uri = AWSV4Signer.fmt_s3_request_uri(bucket_name, object_name)

        data = data_generator(config.object_size, config.part_number)
        async for _ in data:
            for item in _:
                print("******item {}******".format(item["part_no"]))
            #print("data {}".format(_))
                # print("Part count {}".format(part_count))
                # while part_count != 0:
                print(
                'PUT on {} for object size {} bytes.'.format(
                    config.endpoint + request_uri, config.object_size))

                query_params = urllib.parse.urlencode({'partNumber': item["part_no"], 'uploadId': 'e60f2255-0c39-49de-a3f8-d8d0a5b24e6d'})
                body = ""

                headers = AWSV4Signer(
                    config.endpoint,
                    config.s3_service_name,
                    config.s3_region,
                    config.access_key,
                    config.secret_key).prepare_signed_header(
                    'PUT',
                    request_uri,
                    query_params,
                    body)

                if (headers['Authorization'] is None):
                    print("Failed to generate v4 signature")
                    sys.exit(-1)

                #print('PUT on {}'.format(config.endpoint + request_uri))

                async with session.put(config.endpoint + request_uri,
                    data=item["data"], params=query_params, headers=headers) as resp:
                    http_status = resp.status
                    print("Response of PUT request {} ".format(resp))

                if http_status == 200:
                    print("HTTP status {} OK!".format(http_status))
                else:
                    print("ERROR : BAD RESPONSE! status = {}".format(http_status))
                
                #part_count-=1

loop = asyncio.get_event_loop()
loop.run_until_complete(main())